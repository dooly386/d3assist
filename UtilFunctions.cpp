...
void MouseUp(TMouseButton button)
{
	...
	if(button==mbLeft)
	{
		if(IsKeyPressed(VK_LBUTTON)==false) return;
	...
	if(button==mbRight)
	{
		if(IsKeyPressed(VK_RBUTTON)==false) return;
	...
	if(button==mbMiddle)
	{
		if(IsKeyPressed(VK_MBUTTON)==false) return;
...
{
	if(IsKeyPressed(vcode)==false) return;
